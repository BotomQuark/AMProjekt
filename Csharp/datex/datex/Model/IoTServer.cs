...
namespace datex.Model
{
    public class IoTServer
    {
        ...
        {
            return "http://" + ip + "/dataCS.json";
        }

        private string GetFileUrl_Array()
        {
            return "http://" + ip + "/data_arrayCS.json";
        }
        ...
        public async Task<string> GETwithClient_Array()
        {
            ...
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    responseText = await client.GetStringAsync(GetFileUrl_Array());
                }
            }
            catch (Exception e)
            {
        ...
        public async Task<string> POSTwithClient_send_led(string x, string y, string r, string g, string b)
        {
            ...
            string url = "http://" + ip + "/projekt/setled.php?LED00=[" + x + "," + y + "," + r + "," + g + "," + b + "]";
            try
            {
                using (HttpClient client = new HttpClient())
                {
        ...
        public async Task<string> POSTwithClient_send_led_clear(string[] led, string[] x, string[] y, string[] r, string[] g, string[] b)
        {
            string responseText_send = null;

            string url = "http://" + ip + "/projekt/setled.php?LED" + led[0] + "=[" + x[0] + "," + y[0] + "," + r[0] + "," + g[0] + "," + b[0] + "]";

            for (int i = 1; i < 64; i++)
            {
                url = url + "&LED" + led[i] + "=[" + x[i] + "," + y[i] + "," + r[i] + "," + g[i] + "," + b[i] + "]";
            }
            try
            {
                using (HttpClient client = new HttpClient())
